...
const getFileFromAmbiguousArg = ambiguousArg => {
  try {
    const foundFile = fs.readFileSync(path.resolve('.', ambiguousArg));
...
const makeSecondRequest = ({
  ...
  request({
    uri: uri.replace(':3001', ':3000'),
    method: 'put',
    body: File,
    headers: { 'Content-Type': contentType, 'Content-MD5': checksum, 'X-CSRF-Token': 'zA==--PgL4K2/86JKW0vok--0SCP3DMRmJbIN/+kK9AHQg==' }
  }, (error, response) => {
    const { statusCode, statusMessage } = response || {};
...
const uploadDownlinkedFile = async ({
  ...
  const { direct_upload: { url: uri } = {}, signed_id } = firstResponse;
  // const { url } = direct_upload;

  await makeSecondRequest({ checksum, contentType, uri, File });
